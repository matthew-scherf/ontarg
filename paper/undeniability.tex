...
\subsection{Self-refutation and retorsion}

Mackie distinguishes absolute, operational, and pragmatic self-refutation
\cite{mackie1964}. Theorem~\ref{thm:discrimination} is absolute once the
tokening difference is an antecedent: it refutes a conjunction in
\(\mathrm{Prop}\). Placing the tokening fact in the hypothesis converts a
pragmatic phenomenon into a propositional one, in line with Castagnoli's case
against assimilating ancient self-refutation to the pragmatic model
\cite{castagnoli2010}. The ancient background is the \emph{peritrope} of the
\emph{Theaetetus} and Aristotle's elenctic defence of non-contradiction
\cite{plato,aristotle,burnyeat1976}. Hintikka's existential inconsistency ---
the impossibility of a sentence's being true as tokened by the one who tokens
it --- is the nearest modern analogue with the tokening left partly implicit
\cite{hintikka1962}; here the tokening is a parameter of the theorem.

Kaplan's separation of validity in every context from necessity marks the same
boundary the obstruction makes precise for succession: a sentence can hold
whenever tokened and still fail to be necessary \cite{kaplan1989}.

\subsection{Transcendental arguments and final justification}

Stroud's limit --- that transcendental arguments reach the denier's commitments
rather than the world --- is the template \cite{stroud1968}.
...
moral philosophy is among the targets of the obstruction wherever such an
argument requires temporal or dynamical import from undeniability alone
\cite{illies2003}.
...
temporal case: the conditionals are proved, \(P\) is independent of every claim
true at the two-model witness, and the obstruction explains why an undeniable
temporal bridge is unavailable. The acceptance is local to succession;
occurrence remains on the ambitious side. The standing granted to a single
realized instance of \(P\) follows Moore; Strawson's naturalism --- on which
skeptical doubts are idle because the commitments they question are not up for
choice --- is the informal support for treating that instance as beyond serious
dispute \cite{moore1925,strawson1985}.

\subsection{Hegel and the importation charge}

Trendelenburg charged that Hegel's derivation of movement in the Logic imports
intuition \cite{trendelenburg1870,hegel1832}.
Theorem~\ref{thm:obstruction} is a formal analogue under a succession-blind
claim language: such a language entails no succession.
Section~\ref{sec:dilemma} defends the regimentation by showing that the tensed
alternative reproduces the same accounting on either horn. The paper does not
offer a reading of the Logic; it records that a presuppositionless
combinatorial beginning, if regimented as here, stops short of succession.
Kierkegaard's separation of coming-into-existence from essence, and Schelling's
demand that positive philosophy add existence to what negative philosophy
derives, mark the same gap that \(P\) occupies
\cite{kierkegaard1844,schelling1994}. Dialetheist readings of the opening
\cite{priest2006} conflict with the constructive metalogic used here;
non-dialetheic analytic readings are available \cite{ficara2020,berto2007}.

\subsection{Temporal ontology}

McTaggart's C-series is ordered without direction \cite{mctaggart1908}; Farr's
C-theory retains that refusal \cite{farr2020}.
Theorem~\ref{thm:orientation} shows the refusal is stable under relabeling of
loci: no invariant succession is oriented. The world / tensed-world distinction
used here is broader than the C-/B-/A-series taxonomy, since it adds an
arbitrary binary relation to an unordered base of loci.

Even when a run is granted, the formal content is at-at variation along a
succession, not A-theoretic passage
\cite{williams1951,smart1949,broad1923,prior1959}. Skow's moving spotlight
names the further commitment precisely \cite{skow2015}. Mellor's
token-reflexive account is the closest match to the \(\mathrm{tok}/h\)
signature and imposes the causal scope condition of Section~\ref{sec:indep}: if
causal order is added to the signature and determines temporal order, \(P\) may
become derivable \cite{mellor1998}. Grünbaum's mind-dependent becoming places
temporal content in awareness of events \cite{grunbaum1967}, adjacent to a
tokening-based account but silent on the obstruction. Fine's fragmentalism
gives tensed facts without a global standpoint \cite{fine2005}; the still and
running pair has no fragment structure, so the present framework has nothing to
say to a fragmentalist. Putnam's relativity argument concerns
frame-independent succession between spacelike-separated events
\cite{putnam1967}; successive readings by one reader are timelike and are not
touched by that argument.

\subsection{Laws of Form and structural neighbours}

Spencer-Brown introduces an oscillating value to solve an equation with no
ordinary solution and reads it as time \cite{spencerbrown1969}. Varela answers
with a third autonomous value \cite{varela1975}; Kauffman reads the oscillation
as an eigenform \cite{kauffman1987,kauffmanvarela1980}. Those proposals address
the structural half summarized in Section~\ref{sec:structure}. The present
paper does not re-derive that half; it separates it from succession and assigns
the latter to \(P\). Under mark-respecting equivariance, a third value does not
yield mark-moving dynamics (Section~\ref{sec:structure}, item~1); under full
equivariance a third value forces the identity. Whether an autonomous third
value or an infinite eigenform counts as a value of the carrier is left to
those programmes; on a two-valued carrier with decidable equality they do not.

\subsection{Verified ontological arguments}

Automated and interactive treatments of Anselm and of Gödel typically place
existential content in the axioms and check the derivation
\cite{oppenheimerzalta2011,benzmuller2014,sobel1987,benzmuller2016,anderson1990}.
Here no axiom is declared. Existential temporal content sits in \(P\) and in
the ostensive discharge of discrimination; the verified material is the
surrounding conditional and limit structure. The metatheoretic expectation that
a closed normal term of type \(\mathrm{Prop}\to\B\) is constant --- used in
Section~\ref{sec:discrimination} to locate detachment outside the system ---
follows the expected behaviour of Lean's axiom-free calculus
\cite{carneiro2019}; a full canonicity theorem for the implemented Lean~4
kernel is not assumed.
...
\bibitem{anderson1990} C.\ A.\ Anderson, ``Some Emendations of G\"odel's
Ontological Proof,'' \emph{Faith and Philosophy} 7(3), 1990, 291--303.

\bibitem{apel1980} K.-O.\ Apel, \emph{Towards a Transformation of Philosophy},
trans.\ G.\ Adey and D.\ Frisby, Routledge \& Kegan Paul, 1980.

\bibitem{aristotle} Aristotle, \emph{Metaphysics} \(\Gamma\).3--4; in
\emph{The Complete Works of Aristotle}, ed.\ J.\ Barnes, Princeton University
Press, 1984.
...
\bibitem{berto2007} F.\ Berto, ``Hegel's Dialectics as a Semantic Theory: An
Analytic Reading,'' \emph{European Journal of Philosophy} 15(1), 2007, 19--39.

\bibitem{broad1923} C.\ D.\ Broad, \emph{Scientific Thought}, Kegan Paul, 1923.

\bibitem{burnyeat1976} M.\ F.\ Burnyeat, ``Protagoras and Self-Refutation in
Plato's \emph{Theaetetus},'' \emph{Philosophical Review} 85(2), 1976, 172--195.

\bibitem{carneiro2019} M.\ Carneiro, \emph{The Type Theory of Lean}, MSc
thesis, Carnegie Mellon University, 2019.

\bibitem{castagnoli2010} L.\ Castagnoli, \emph{Ancient Self-Refutation},
...
\bibitem{ficara2020} E.\ Ficara, \emph{The Form of Truth: Hegel's Philosophical
Logic}, De Gruyter, 2020.

\bibitem{fine2005} K.\ Fine, ``Tense and Reality,'' in \emph{Modality and
Tense: Philosophical Papers}, Oxford University Press, 2005, 261--320.
...
\bibitem{kauffmanvarela1980} L.\ H.\ Kauffman and F.\ J.\ Varela, ``Form
Dynamics,'' \emph{Journal of Social and Biological Structures} 3(2), 1980,
171--206.

\bibitem{kierkegaard1844} S.\ Kierkegaard, \emph{Philosophical Fragments}
(1844), the ``Interlude''; trans.\ H.\ V.\ Hong and E.\ H.\ Hong, Princeton
...
\bibitem{plato} Plato, \emph{Theaetetus} 170e--171c; in \emph{Plato: Complete
Works}, ed.\ J.\ M.\ Cooper, Hackett, 1997.

\bibitem{priest2006} G.\ Priest, \emph{In Contradiction}, 2nd ed., Oxford
...
\bibitem{schelling1994} F.\ W.\ J.\ Schelling, \emph{On the History of Modern
Philosophy}, trans.\ A.\ Bowie, Cambridge University Press, 1994.
...
\bibitem{sobel1987} J.\ H.\ Sobel, ``G\"odel's Ontological Proof,'' in
\emph{On Being and Saying}, ed.\ J.\ J.\ Thomson, MIT Press, 1987, 241--261.
...
\bibitem{strawson1985} P.\ F.\ Strawson, \emph{Skepticism and Naturalism: Some
Varieties}, Columbia University Press, 1985.
