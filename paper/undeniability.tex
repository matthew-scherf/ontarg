...
exactly the same claims; in particular, a still structure and a
running-capable one are claim-indiscernible. Consequently, no undeniable claim
--- no claim true at every world that discriminates its denial --- can entail
running. The same two-model exhibition shows that an oriented succession
premise \(P\) is independent of the claim language relative to the signature
\((T,h,\mathrm{tok})\). On loci with decidable equality, moreover, every
...
the world signature, \(P\) is independent of every claim. A further theorem
locates the directional content of \(P\): on loci with decidable equality,
invariant succession is always symmetric (Section~\ref{sec:orientation}).

The structural facts about a two-valued carrier --- that equivariant laws move
a point exactly on two elements, that the excluding structureless law is
complement, that complement has no fixed point and yields a unique period-two
orbit --- are recorded in Section~\ref{sec:structure} as background, with
proofs in the accompanying Lean development. They are not used to derive
succession. The paper's thesis is the limit, not an existence proof for
becoming.
...
The corollary is a rebuttal schema. Given a candidate axiom offered as an
undeniable bridge from discrimination to succession, instantiate a still world
that discriminates its denial (e.g.\ inscriptions on a page with empty
succession). The candidate fails. The schema does not close the gap for
candidates that are never so tokened; it shows that undeniability, as defined,
cannot be the bridge.
...
By Proposition~\ref{prop:freeze}, \(P\) fails throughout the still structure
over any world and holds (in its ordering half) throughout the total
structure over the same world. By Corollary~\ref{cor:nobridge}, no undeniable
replacement is available. On the act reading, a single instance of \(P\) is an
ordinary report of reading order: the denial was read, then the assertion, and
not conversely \cite{moore1925}. Warrant for one instance does not automatically
extend to an infinite family of instances.
...
\end{enumerate}
Hence no claim entails \(P\) and no claim refutes \(P\). The same exhibition
works for oriented \(P\) over a world whose tokening separates a chosen pair.
The full chain hypothesis is realized over a world with infinitely many loci
(e.g.\ successor on \(\mathbb{N}\) with alternating values).
\end{proposition}

\begin{proof}
Take \(T=\B\), \(h=\mathrm{id}\), and a constant tokening for the still/total
pair; Proposition~\ref{prop:freeze} gives~(iii). For oriented independence, take
succession to hold of \((\bot,\top)\) alone. For the chain, take \(T=\mathbb{N}\)
with the successor relation and alternating reading.
\end{proof}
...
\label{prop:invsucc}
An invariant succession takes one truth-value on the diagonal and one off it.
The four possibilities are: empty, equality, disagreement, and the total
relation. Equality and the empty relation carry no run. An invariant succession
that fails somewhere on the diagonal and holds of some off-diagonal pair is
disagreement.
\end{proposition}

\begin{proof}[Proof sketch]
One transposition carries any diagonal pair to any other, so the diagonal is
uniform. Two transpositions carry any off-diagonal pair to any other, so the
off-diagonal is uniform when at least two loci exist. The four combinations of
diagonal/off-diagonal truth-values are the four relations named. A succession
contained in equality identifies consecutive terms of any sequence, against
injectivity. The last claim: failure on the diagonal and success off it force
...
\item \emph{Arena.} On a carrier with decidable equality, an equivariant
endofunction (commuting with every transposition) moves some point if and only
if the carrier has exactly two elements. The same equivalence holds for
mark-respecting equivariance (commutation with transpositions fixing a
distinguished mark).
...
Proposition~\ref{prop:independence} & \lf{premise\_independent}, \ldots \\
...
does not yield succession: undeniability and running are incompatible in the
presence of a still discriminating world, and such worlds are available by
freeze. The succession premise \(P\) is independent of every claim relative to
the retorsive signature, and its orientation clause is invisible to invariant
structure. The a priori core is therefore the conditionals together with this
limit. Time, if wanted, is paid for separately.
